...
import { Camera, Search, TrendingDown, BarChart3, Users, Package, AlertCircle, Image as ImageIcon, X, Share } from 'lucide-react';
...
const PriceScannerApp = () => {
  ...
  const startQuaggaScan = async () => {
    ...
    Quagga.onDetected((result) => {
      if (result && result.codeResult && result.codeResult.code) {
        ...
        // Stop QuaggaJS first to prevent multiple detections
        Quagga.stop();
  ...
  // BarcodeDetector API Scanner for Android/Chrome
  ...
  // Main scan function - chooses the right scanner
  const startScan = () => {
    if (isIOS() || !('BarcodeDetector' in window)) {
      // Use QuaggaJS for iOS or browsers without BarcodeDetector
      console.log('Using QuaggaJS scanner');
      startQuaggaScan();
    } else {
      // Use native BarcodeDetector API for Android
  ...
  const submitPrice = async () => {
    ...
    try {
      ...
      // Success!
      alert('Prix enregistre avec succes! Merci pour votre contribution.');
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white min-h-screen">
      {/* Header */}
      <div className="bg-gradient-to-r from-orange-500 via-red-500 to-pink-500 text-white p-6 shadow-lg">
        <h1 className="text-2xl font-bold mb-2">Vie chere en Martinique</h1>
        ...
        {activeTab === 'scan' && (
          <div className="space-y-4">
            ...
            {!scanning ? (
              ...
              <div className="space-y-4">
                {/* Scanner container - used by both QuaggaJS and BarcodeDetector */}
                <div className="relative bg-black rounded-lg overflow-hidden" style={{ height: '300px' }}>
                  {/* QuaggaJS scanner container */}
                  <div
                    ref={scannerRef}
                    className="w-full h-full"
                    style={{ position: 'relative' }}
                  >
                    {/* Video element for BarcodeDetector API */}
                    <video
                      ref={videoRef}
                      ...
                      autoPlay
                      muted
                      style={{ display: isIOS() || !('BarcodeDetector' in window) ? 'none' : 'block' }}
                    />
                  </div>
                  <div className="absolute inset-0 border-2 border-orange-400 m-8 rounded-lg pointer-events-none">
                    <div className="absolute top-0 left-0 right-0 bg-orange-500 text-white text-xs py-1 px-2 text-center font-semibold">
                    ...
                    <div key={productName} className="bg-white border border-gray-200 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-sm">
                        <div>
                          ...
                          <div className="font-semibold text-green-600">{stats.min.toFixed(2)}EUR</div>
                        </div>
                        <div>
                          <div className="text-gray-500 text-xs">Moyen</div>
                          <div className="font-semibold">{stats.avg.toFixed(2)}EUR</div>
                        </div>
                        <div>
                          <div className="text-gray-500 text-xs">Max</div>
                          <div className="font-semibold text-red-600">{stats.max.toFixed(2)}EUR</div>
                        </div>
                      </div>
