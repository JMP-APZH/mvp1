...
None blocking. All migrations to date (`bqp_vote_stats_fix_migration.sql`, `barcode_audit_migration.sql`, `trim_city_cleanup_migration.sql`, `categories_admin_insert_migration.sql`) — check each file's header comment for apply status as of the date you're reading this; the two newest were written 2026-07-21 and may not yet be applied.
...
- Category coverage is currently low (5/23 products) since assigning a category during capture is optional — the new category/store filters will look sparse until more scans are categorized.
...
- `src/components/CategoryManager.jsx` — Admin-only category creation (Console Admin sub-tab).
